                    ...
                    <section id="ecommerce-products" class="grid-view">
                        @foreach($users as $user)
                            <div class="card card-profile">
                                ...
                                <div class="card-body">
                                    <h3><a href="{{ route('admin.permission.register.seller.details',$user->id)}}">{{ $user->name }}</a> </h3>
                                    @foreach($user->cats as $cat)
                                    <h6 class="text-muted">{{ $cat->title }}</h6>
                                    @endforeach
                                    ...
                                    <div class="align-items-center">
                                       <a href="{{ route('admin.permission.register.seller.details',$user->id)}}" class="btn btn-primary">Enter</a>
                                    </div>
                                </div>
                            </div>
                        @endforeach
