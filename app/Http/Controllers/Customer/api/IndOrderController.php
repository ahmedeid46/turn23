...
class IndOrderController extends Controller
{
    ...
    function allOrder(){
        $orders =IndOrder::Where('customer_id',auth('customer-api')->user()->getAuthIdentifier())->with('product')->get();
    ...
    function index(Request $request){
        ...
        try {
            ...
            $order->count = $request->count." ".$request->count_unit;
            $order->customer_id = auth('customer-api')->user()->getAuthIdentifier();
            $order->price =null;
            $order->origin = $request->origin;
            $order->matrial_constraction = $request->matrial_constraction;
            $order->dimension = $request->dimension." ".$request->dimension_unit;
            $order->schedule = $request->schedule;
            $order->capacity = $request->capacity." ".$request->capacity_unit;
            $order->ingress = $request->ingress;
            $order->rpm = $request->rpm;
            $order->pressure = $request->pressure." ".$request->pressure_unit;
            $order->tempratuve = $request->tempratuve." ".$request->tempratuve_unit;
            $order->equivelient = $request->equivelient;
            if ($request->hasFile('specialRequirement')){
                $order->specialRequirement = $this->orderUploadFile('ind_SR_',$request->specialRequirement,'specialRequirement');
        ...
        return $this->returnSuccess(['Order Added'],'',200);
            ...
            return $this->returnSuccess(['Server error',$e->getMessage()],500);
        }
    ...
    function payment(Request $request){
        ...
        $orders =IndOrder::firstWhere('id',$request->order_id);
    ...
    function tenders(){
        $data['tenders'] = IndTender::with('tender_price')->where('customer_id',auth('customer-api')->user()->getAuthIdentifier())->get();
        return $this->returnSuccess('',$data);
    }
    function tender_request(Request $request){
        $validated = Validator::make($request->all(), [
            "tender_file"=>'required|file',
        ]);
        if($validated->fails()){
            return $this->returnError($validated->errors()->all(),'400');
        }
        try{
            $tender = new IndTender;
            $tender->customer_id = auth('customer-api')->user()->id;
    ...
    function tender_delete(Request $request){
        $validated = Validator::make($request->all(), [
            "tender_id"=>'required|exists:ind_tenders',
        ]);
        if($validated->fails()){
            return $this->returnError($validated->errors()->all(),'400');
        }
