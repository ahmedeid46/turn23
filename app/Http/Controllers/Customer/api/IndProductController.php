...
class IndProductController extends Controller
{
    ...
    function product(Request $request){
        ...
        $products = IndAllProduct::with('adminproduct')->where('id',$request->product_id)->get();
        foreach ($products as $allproduct) {
            $allproduct['cover'] = route('product.imgs', [encrypt('cover'), encrypt($allproduct['cover'])]);
            if($allproduct->adminproduct != null) {
    ...
    function slider(Request $request){
        $products = IndAdminProduct::latest()->take(5)->get();
        $sliders = [];
